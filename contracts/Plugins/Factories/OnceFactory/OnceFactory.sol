...
import { IAccessControl } from "../../Base/AccessControl/IAccessControl.sol";
...
contract OnceFactory is IOncePlugin, AccessControlEnforcer {
    ...
    /**
     * @notice Internal function for cleaning up initialization process
     * @param _once the newly deployed ONCE
     * @param _callback the callback if any used during initialization
     * @dev This is called at the end of the public deployOnce and deployOnceWithSimpleSalt functions
     * @dev renounces and revokes roles from the callback and OnceFactory addresses
     */
    function cleanUpInitialization(address _once, IOnceDeploymentCallback _callback) internal {

        if(address(_callback) != address(0)) {
            IAccessControl(_once).revokeRole(OnceStorage.ONCE_UPDATE_ROLE, address(_callback));
            IAccessControl(_once).revokeRole(OnceStorage.DEFAULT_ADMIN_ROLE, address(_callback));
        }

        IAccessControl(_once).renounceRole(OnceStorage.ONCE_UPDATE_ROLE);
        IAccessControl(_once).renounceRole(OnceStorage.DEFAULT_ADMIN_ROLE);
    }
    ...
    function deployOnce(bytes32 _salt, OnceFactoryStorage.RoleInitializer[] calldata rolesToCreate, OnceFactoryStorage.PluginInitializer calldata pluginsToInstall, IOnceDeploymentCallback _callback) public returns (Once once) {
        ...
        if (address(_callback) != address(0)) {
            IAccessControl(address(once)).grantRole(OnceStorage.ONCE_UPDATE_ROLE, address(_callback));
            IAccessControl(address(once)).grantRole(OnceStorage.DEFAULT_ADMIN_ROLE, address(_callback));
            _callback.onceDeployed(once, _salt, rolesToCreate, pluginsToInstall);
        }
        cleanUpInitialization(address(once), _callback);
    }
