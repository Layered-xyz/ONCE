import { Signer, BigNumberish, BaseContract, ethers, VoidSigner } from "ethers";
...
export const signHash = async (signer: Signer, hash: string): Promise<SafeSignature> => {
    const typedDataHash = ethers.utils.toUtf8Bytes(hash); // ethers.getBytes
    const signerAddress = await signer.getAddress();
    return {
        signer: signerAddress,
...
export const executeTxWithSigners = async (safe: Safe, tx: SafeTransaction, signers: Signer[], overrides?: any) => {
    const safeAddress = safe.address;
    const sigs = await Promise.all(signers.map((signer) => safeSignTypedData(signer as VoidSigner, safeAddress, tx)));
